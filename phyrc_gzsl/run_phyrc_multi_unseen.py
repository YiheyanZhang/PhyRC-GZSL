...
from phyrc_gzsl.utils.config import load_config, set_unseen_classes, class_id_lists


ROOT = Path(__file__).resolve().parents[1]
OUTPUT = ROOT / "phyrc_gzsl/checkpoints/multi_unseen"
...
DATASETS = {
    ...
    "longkou": {
        "config": "phyrc_gzsl/configs/longkou_p1.yaml",
        "attributes": "phyrc_gzsl/data/processed/LongKou_structured_attributes.json",
        "unseen_classes": [3, 6],
    },
}
...
def build_jobs() -> list[dict]:
    ...
    for dataset, settings in DATASETS.items():
        for seed in range(42, 47):
            folder = f"phyrc_gzsl/checkpoints/multi_unseen/{dataset}/seed{seed}"
            jobs.append({
                "dataset": dataset,
                "seed": seed,
...
def _train(job: dict) -> None:
    command = [
        sys.executable, "-m", "phyrc_gzsl.train_spectral_mae",
        "--config", job["config"],
