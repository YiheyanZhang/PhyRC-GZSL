...
from phyrc_gzsl.run_phyrc_multi_unseen import _atomic_json, _partition


ROOT = Path(__file__).resolve().parents[1]
OUTPUT = ROOT / "phyrc_gzsl/checkpoints/multi_unseen/baselines"
...
def build_jobs() -> list[dict]:
    full_jobs = json.loads(
        (ROOT / "phyrc_gzsl/checkpoints/multi_unseen/manifest.json").read_text(encoding="utf-8")
    )["jobs"]
    return [
        {
            **job,
            "method": method,
            "result": f"phyrc_gzsl/checkpoints/multi_unseen/baselines/{method}/{job['dataset']}/seed{job['seed']}.json",
        }
...
def main() -> None:
    ...
    _atomic_json(OUTPUT / "manifest.json", {
        "source_manifest": "phyrc_gzsl/checkpoints/multi_unseen/manifest.json", "jobs": jobs,
    })
    if not args.execute:
