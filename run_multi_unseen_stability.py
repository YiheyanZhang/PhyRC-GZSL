...
OUTPUT = ROOT / "checkpoints/multi_unseen_stability"
    ...
    for dataset, settings in DATASETS.items():
        for group, unseen in enumerate(settings["groups"], start=1):
            for seed in range(42, 47):
                folder = f"checkpoints/multi_unseen_stability/{dataset}/group{group}/seed{seed}"
                jobs.append({
...
def _train(job: dict) -> None:
    subprocess.run([
        sys.executable, "-m", "train_spectral_mae", "--config", job["config"],
