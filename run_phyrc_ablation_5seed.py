...
from evaluate_phyrc import evaluate_stage2_split


ROOT = Path(__file__).resolve().parent
OUTPUT = ROOT / "checkpoints/ablations/phyrc"
VARIANTS = (
    "no_relational_prototype", "no_cross_fitting",
    "no_dual_evidence", "no_risk_constraint",
)
DATASETS = {
    "paviau": ("configs/paviau_p1.yaml", "data/processed/PaviaU_structured_attributes.json", range(1, 10)),
    "houston": ("configs/houston_p1.yaml", "data/processed/Houston_structured_attributes.json", range(1, 16)),
    "longkou": ("configs/longkou_p1.yaml", "data/processed/LongKou_structured_attributes.json", range(1, 10)),
    "indian_pines": ("configs/indian_pines_p1.yaml", "data/processed/IndianPines_structured_attributes.json", range(1, 17)),
}
...
def checkpoint(dataset: str, seed: int, unseen: int) -> Path:
    if dataset == "indian_pines" and seed != 42:
        return ROOT / f"checkpoints/indian_pines_submission/single/seed{seed}/backbone_s{unseen}.pt"
    name = f"{dataset}_p1_backbone_s{unseen}.pt"
    relative = f"checkpoints/{name}" if seed == 42 else f"checkpoints/multiseed/{dataset}/seed{seed}/{name}"
...
def full_result_paths(dataset: str, seed: int) -> list[Path]:
    if dataset == "indian_pines":
        return [ROOT / f"checkpoints/indian_pines_submission/single/seed{seed}/phyrc_s{unseen}.json" for unseen in range(1, 17)]
    if seed == 42:
        return [ROOT / f"checkpoints/{dataset}_phyrc_single_unseen.json"]
    return [ROOT / f"checkpoints/multiseed/{dataset}/seed{seed}/{dataset}_phyrc.json"]
