...
from run_phyrc_multi_unseen import _atomic_json
from utils.config import class_id_lists, load_config, set_unseen_classes


ROOT = Path(__file__).resolve().parent
OUTPUT = ROOT / "checkpoints/indian_pines_submission"
CONFIG = "configs/indian_pines_p1.yaml"
ATTRIBUTES = "data/processed/IndianPines_structured_attributes.json"
...
PILOT = {
    "phyrc": "checkpoints/indian_pines_phyrc_single_unseen.json",
    "eszsl": "checkpoints/indian_pines_pilot/eszsl_seed42.json",
    "sae": "checkpoints/indian_pines_pilot/sae_seed42.json",
    "f_clswgan": "checkpoints/indian_pines_pilot/f_clswgan_seed42.json",
    "cada_vae": "checkpoints/indian_pines_pilot/cada_vae_seed42.json",
    "free": "checkpoints/indian_pines_pilot/free_seed42.json",
}


def _backbone(protocol: str, seed: int, unseen: list[int]) -> str:
    if protocol == "single" and seed == 42:
        return f"checkpoints/indian_pines_p1_backbone_s{unseen[0]}.pt"
    suffix = f"backbone_s{unseen[0]}.pt" if protocol == "single" else "backbone.pt"
    return f"checkpoints/indian_pines_submission/{protocol}/seed{seed}/{suffix}"
...
def _evaluation(protocol: str, method: str, seed: int, unseen: list[int]) -> dict:
    ...
    return {
        ...
        "result": f"checkpoints/indian_pines_submission/{protocol}/seed{seed}/{method}_{tag}.json",
    }
...
def _train(path: Path, unseen: list[int], seed: int) -> None:
    subprocess.run([
        sys.executable, "-m", "train_spectral_mae", "--config", CONFIG,
