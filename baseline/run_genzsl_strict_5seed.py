...
from baseline.evaluate_genzsl_strict import OFFICIAL_COMMIT, STRICT_GENZSL, STRICT_PROTOCOL, evaluate_split
from diagnose_rstd_ot import resolve_project_path

ROOT = Path(__file__).resolve().parents[1]
OUTPUT = ROOT / "checkpoints/baselines/genzsl_strict"
DATASETS = {
...
def checkpoint_pattern(dataset: str, seed: int) -> str:
    ...
    return f"checkpoints/{name}" if seed == 42 else f"checkpoints/multiseed/{dataset}/seed{seed}/{name}"
